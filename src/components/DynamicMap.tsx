...
export const DynamicMap: React.FC<DynamicMapProps> = ({
    ...
    const propertiesWithCoords = useMemo(() =>
        (manualProperties || PROPERTIES).filter(p => p.lat && p.lng),
        [manualProperties]
    );
    ...
    console.log('DynamicMap: Initializing...', { hasToken: !!MAPBOX_TOKEN, tokenPrefix: MAPBOX_TOKEN?.substring(0, 5) });

    return (
        <div className="w-full h-full min-h-[500px] relative group/map bg-luxury-black/20 border border-white/5 rounded-3xl overflow-hidden">
            {showFallback ? (
                <MaltaSVGFallback
...
const MapBoxView = ({
    ...
    const [viewport, setViewport] = useState({
        latitude: manualCenter ? manualCenter[1] : 35.9122,
        longitude: manualCenter ? manualCenter[0] : 14.5042,
        zoom: manualZoom || 11
    });
