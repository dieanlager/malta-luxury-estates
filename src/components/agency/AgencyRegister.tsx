    ...
    <main className="min-h-screen bg-luxury-black flex items-center justify-center px-6">
      <div className="w-full max-w-md">
        <div className="text-center mb-12">
          ...
          <p className="text-white/60 text-sm">Join Malta Luxury Real Estate portal</p>
        </div>
        <form onSubmit={handleSubmit} className="glass-card p-10 rounded-3xl border border-white/10 space-y-5">
          ...
          {[['agencyName','Agency Name','Malta Properties Ltd'],['name','Contact Name','Your name'],['email','Email','agency@example.com']].map(([key, label, ph]) => (
            <div key={key}>
              <label className="text-[10px] uppercase tracking-widest text-white/60 mb-2 block">{label}</label>
          ...
          <p className="text-center text-xs text-white/60">Already registered? <Link href="/agency/login" className="text-gold">Sign in</Link></p>
        </form>
      </div>
    </main>
  );
