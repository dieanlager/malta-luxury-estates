...
import {
  ...
  RefreshCw, Eye, EyeOff, Star, Loader2, ArrowLeft, ChevronLeft,
} from 'lucide-react';
          ...
          <div>
            ...
            {!loadingList && props.length === 0 && (
              <div className="glass-card border border-white/10 rounded-2xl p-12 text-center">
                <p className="text-white/30 text-sm mb-4">Brak ofert. Dodaj pierwszą nieruchomość!</p>
                    ...
                    <div className="flex-1 px-4 py-3 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-white/40 text-xs mt-0.5">{p.location_text} · {fmtPrice(p.price)} · {p.listing_type === 'rent' ? 'Wynajem' : 'Sprzedaż'}</p>
                        </div>
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          <button onClick={() => setConfirmDel(p)} title="Usuń" className="w-8 h-8 rounded-lg flex items-center justify-center text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
      ...
      {confirmDel && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-card border border-white/10 rounded-2xl p-6 max-w-sm w-full">
            ...
            <p className="text-white/30 text-xs mb-6">Tej operacji nie można cofnąć.</p>
            <div className="flex gap-3">
