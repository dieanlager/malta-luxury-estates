...
const AdminPage: React.FC = () => {
  ...
  const fetchProps = useCallback(async () => {
    ...
    } catch (e: any) { toast('BĹ‚Ä…d Ĺ‚adowania: ' + e.message, false); }
  ...
  const handleScrape = async () => {
    if (!url.trim()) { toast('Wklej URL nieruchomoĹ›ci', false); return; }
    setScraping(true);
    try {
      ...
      setBlobPreviews([]);
      toast('Dane pobrane. Wgraj wĹ‚asne zdjÄ™cia poniĹĽej.');
    } catch (e: any) { toast('BĹ‚Ä…d: ' + e.message, false); }
  ...
  const processFiles = async (files: FileList | File[]) => {
    ...
    setUploading(false);
    if (errors > 0) toast('Wgrano ' + serverUrls.length + ', bĹ‚Ä…d ' + errors, serverUrls.length > 0);
  ...
  const handlePublish = async () => {
    if (!form.title) { toast('Wpisz tytuĹ‚', false); return; }
    ...
    return (
      <main className="min-h-screen bg-luxury-black flex items-center justify-center px-6">
        <div className="glass-card border border-white/10 rounded-2xl p-8 w-full max-w-sm">
          ...
          <p className="text-white/30 text-sm mb-6">Malta Luxury Real Estate</p>
          <input
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-white/30 text-sm mt-1">Malta Luxury Real Estate</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="Wyloguj"
              className="ml-2 flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/10 text-white/40 hover:text-red-400 hover:border-red-500/30 text-xs font-bold uppercase tracking-widest transition"
            >
        ...
        {view === 'list' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <p className="text-white/30 text-sm tabular-nums">{props.length} ofert w bazie</p>
              <button onClick={fetchProps} disabled={loadingList} className="flex items-center gap-1.5 text-white/30 hover:text-white text-xs transition">
                <RefreshCw size={12} className={loadingList ? 'animate-spin' : ''} /> OdĹ›wieĹĽ
              </button>
            </div>

            {loadingList && (
              <div className="flex items-center justify-center h-32 text-white/30">
                <Loader2 size={20} className="animate-spin mr-2" /> Ĺadowanieâ€¦
              </div>
            )}

            {!loadingList && props.length === 0 && (
              <div className="glass-card border border-white/10 rounded-2xl p-12 text-center">
                <p className="text-white/30 text-sm mb-4">Brak ofert. Dodaj pierwszÄ… nieruchomoĹ›Ä‡!</p>
                    ...
                    <div className="flex-1 px-4 py-3 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-white/40 text-xs mt-0.5">{p.location_text} Â· {fmtPrice(p.price)} Â· {p.listing_type === 'rent' ? 'Wynajem' : 'SprzedaĹĽ'}</p>
                        </div>
                        <div className="flex items-center gap-1 flex-shrink-0">
                          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider border ${isActive ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/20' : 'bg-white/5 text-white/30 border-white/10'}`}>
                          ...
                          <button onClick={() => toggleStatus(p)} title={isActive ? 'Wstrzymaj' : 'Aktywuj'} className="w-8 h-8 rounded-lg flex items-center justify-center text-white/30 hover:text-white hover:bg-white/5 transition-colors">
                            {isActive ? <EyeOff size={14} /> : <Eye size={14} />}
                          </button>
                          <button onClick={() => startEdit(p)} title="Edytuj" className="w-8 h-8 rounded-lg flex items-center justify-center text-white/30 hover:text-white hover:bg-white/5 transition-colors">
                            <Edit2 size={14} />
                          </button>
                          <button onClick={() => setConfirmDel(p)} title="UsuĹ„" className="w-8 h-8 rounded-lg flex items-center justify-center text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
          ...
          <div>
            ...
            {editId && (
              <div className="mb-5 px-4 py-3 bg-gold/10 border border-gold/20 rounded-xl text-gold text-sm flex items-center gap-2">
                ...
                <button onClick={() => { setView('list'); setEditId(null); }} className="ml-auto text-xs text-white/40 hover:text-white transition-colors underline">
                  Anuluj
                </button>
              </div>
            )}
            ...
            {!editId && (
              <div className={card}>
                ...
                <p className="text-white/30 text-xs mb-4">Automatycznie pobiera dane z Alliance.mt lub innej strony partnera.</p>
            ...
            <div className={card}>
              <p className="text-[10px] uppercase tracking-[0.2em] text-gold font-bold mb-5">
                {editId ? 'Edytuj dane' : 'Krok 2 â€” SzczegĂłĹ‚y'}
              </p>

              <div className="grid grid-cols-2 gap-4 mb-4">
                {/* Title */}
                <div className="col-span-2">
                  <label className={lbl}>TytuĹ‚</label>
                ...
                {/* Baths */}
                <div>
                  <label className={lbl}>Ĺazienki</label>
                ...
                <div>
                  ...
                  <select value={form.listing_type} onChange={e => set('listing_type', e.target.value)} className={inp}>
                    <option value="sale">Na sprzedaĹĽ</option>
              ...
              <div className="flex flex-wrap gap-4 mb-5">
                {([['is_seafront', 'Sea View / Seafront'], ['has_pool', 'Basen'], ['has_garage', 'GaraĹĽ / Parking']] as [keyof Form, string][]).map(([k, label]) => (
            ...
            <div className={card}>
              ...
              <p className="text-white/30 text-xs mb-4">
                Konwertowane automatycznie do WebP 1920px. PrzeciÄ…gnij kafelki ĹĽeby zmieniÄ‡ kolejnoĹ›Ä‡. Pierwsze = okĹ‚adka.
      ...
      {confirmDel && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-card border border-white/10 rounded-2xl p-6 max-w-sm w-full">
            ...
            <p className="text-white/30 text-xs mb-6">Tej operacji nie moĹĽna cofnÄ…Ä‡.</p>
            <div className="flex gap-3">
